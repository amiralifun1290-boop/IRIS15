...
class RuleBasedAIModel(private val context: android.content.Context? = null) : IntentModel {
    ...
    private fun greetingReply(text: String): String? {
        ...
        val nickname = context?.let { com.iris.assistant.config.UserProfileStore.getNickname(it) }?.takeIf { it.isNotBlank() }
        val nameSuffix = if (nickname != null) " $nickname" else ""
        return when {
            howAreYou.any { text.contains(it) } -> "خوبم، ممنون! تو چطوری؟ چه کمکی از دستم برمیاد؟"
            greetings.any { w -> text == w || text.startsWith("$w ") } -> "سلام$nameSuffix! چه کمکی از دستم برمیاد؟"
